            ...
            if modelo_fmt in ['gfs', 'gefs', 'gefs-estendido', 'cmc-ens']:
                ...
                while True:
                    ...
                    for i in steps:
                        ...
                        url = f'https://nomads.ncep.noaa.gov/pub/data/nccf/com/aigfs/para/aigfs.{data_fmt}/{inicializacao_fmt}/model/atmos/grib2/aigfs.t{inicializacao_fmt}z.sfc.f{i:03d}.grib2'
                ...
                while True:
                    ...
                    for i in steps:
                        ...
                        url = f'https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/para/hgefs.{data_fmt}/{inicializacao_fmt}/ensstat/products/atmos/grib2/hgefs.t{inicializacao_fmt}z.sfc.avg.f{i:03d}.grib2'
        ...
        try:
            ...
            if modo == '24h':
                ...
                if len(files_to_use) > 0:
                    ...
                    for quantil in [0.10, 0.25, 0.50, 0.75, 0.90]:
                        ...
                        for index, time in enumerate(ds_resample.valid_time):
                            ...
                            if anomalia_mensal:
                                ...
                                titulo = gerar_titulo(
                                    ...
                                    data_fim=data_fim,
                                    prefixo_negrito=True,
                                    prefixo=f'p\\ {mes.title()}\\ Q{int(quantil*100)}'
                                )
                ...
                tarefas = [
                    ("15D", dif_01_15d),
                    ("15D-45D", dif_15_final),
                    ("Total", dif_total),
                ]

                for tipo, condicao in tarefas:
                    if not condicao:
                        continue
            ...
            if modo == 'jato_div200':
                ...
                for p_24h in pnmm_24h.tempo:
                    ...
                    plot_campos(
                        ...
                        ds_contour=pnmm_plot[varname]/100,
    ...
    def _processar_precipitacao(self, modo, tipo_plot='tp_db', N_dias=9, **kwargs):
        ...
        try:
            ...
            if modo == '24h':
                ...
                send_whatsapp_message(destinatario=Constants().WHATSAPP_CONDICAO_HIDRICA, mensagem=f'MERGE x Modelos ({self.data.strftime("%d/%m/%Y")})', arquivo=path_painel)
                print(f'Removendo painel ... {path_painel}')
